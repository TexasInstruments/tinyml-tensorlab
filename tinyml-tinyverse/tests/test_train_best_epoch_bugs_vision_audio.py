...
def test_image_classification_train_main_survives_zero_iteration_resume():
    ...
    img_train.dataset_load_state['dataset'] = dataset
    img_train.dataset_load_state['dataset_test'] = dataset
    img_train.dataset_load_state['train_sampler'] = None
    img_train.dataset_load_state['test_sampler'] = None
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(img_train, "load_pretrained_weights", side_effect=lambda model, a, l: model))
...
def _base_audio_args(**overrides):
    args = Namespace(
        quantization=True, dont_train_just_feat_ext='False', load_saved_model='None',
        nas_enabled=False, generic_model=False, nn_for_feature_extraction=False,
        output_int=True, auto_quantization=False, distributed=False, gen_golden_vectors=False,
...
def _set_audio_dataset_load_state(dataset):
    audio_train.dataset_load_state['dataset'] = dataset
    audio_train.dataset_load_state['dataset_test'] = dataset
    audio_train.dataset_load_state['train_sampler'] = None
    audio_train.dataset_load_state['test_sampler'] = None


def _patch_audio_common_pipeline(stack):
    stack.enter_context(patch.object(
    ...
    stack.enter_context(patch.object(audio_train, "load_pretrained_weights", side_effect=lambda model, a, l: model))
...
def test_audio_classification_train_main_survives_zero_iteration_resume():
    args = _base_audio_args()
    _set_audio_dataset_load_state(_FakeAudioClassificationDataset())

    with ExitStack() as stack:
        _patch_audio_common_pipeline(stack)
        audio_train.main(0, args)


def test_audio_classification_train_logs_auc_not_f1_as_auc_roc_score(caplog):
